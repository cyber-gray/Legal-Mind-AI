...
class LegalMindSemanticKernel:
    ...
    async def _call_news_monitor(self, query: str) -> str:
        ...
        try:
            # Try to use the news service directly first
            ...
            if news_items:
                ...
                for i, item in enumerate(news_items, 1):
                    ...
                    response += f"*Source: {item.source} | {item.published_date.strftime('%Y-%m-%d')}*\n"
            ...
            # Fallback to search service if news service isn't available
            try:
                ...
                logger.warning("Search service news fallback failed", error=str(search_error))
            
            # Return fallback message if no news sources work
            return f"""📰 **News Search: {query}**

⚠️  News search temporarily unavailable. This may be due to missing API keys.

**To enable news search:**
1. Add NEWS_API_KEY or BING_SEARCH_KEY to your .env file
2. Run `python setup_environment.py test` to validate configuration

**AI Policy News Sources:**
• AI Act developments
• Regulatory updates
• Industry compliance news
• Technology governance changes

Please configure news APIs for real-time updates."""
            
        except Exception as e:
            logger.error("News monitor plugin failed", error=str(e))
            return f"⚠️  News search unavailable: {str(e)[:100]}..."
